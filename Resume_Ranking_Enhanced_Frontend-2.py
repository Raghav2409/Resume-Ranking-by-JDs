...
st.markdown("""
    ...
    .insight-box {
        ...
        padding: 15px 15px; /* Adjust left padding to add more space */
        margin: 5px 0;
        border-radius: 5px;
    }
    .category-high {
...
@st.cache_data
def load_data():
    """Load and prepare the job descriptions and resumes data."""
    try:
        job_df = pd.read_csv('/Users/raghav/Desktop/Apexon/JD Optimization/job_descriptions_analysis_output v2.csv')
        resume_df = pd.read_csv('/Users/raghav/Desktop/Apexon/JD Optimization/All_Resumes.csv')
        
        # Rename job descriptions for better display
...
if job_df is not None and resume_df is not None:
    ...
    st.error("Unable to load data. Please check your data files and paths.")