...
def render_jd_enhance_page(logger, analyzer, agent):
    ...
    jd_directory = os.path.join(os.getcwd(), "/Users/raghav/Downloads/JDs")
    try:
...
def analyze_resume(file_path):
    """Analyze resume and extract skills, certifications, and tools."""
    import os
    import json
    ...
    try:
        ...
        prompt = (
            "Below is a resume. Analyze it and extract the following information:\n"
            "1. Skills listed, including programming languages, frameworks, tools, and methodologies.\n"
            "2. Certifications mentioned, whether explicitly labeled or described in context.\n"
            "3. Tools or technologies learned or used, inferred from roles, projects, or explicit mentions.\n"
            "4. Additional relevant expertise or proficiencies noted throughout the resume.\n"
            f"\nResume:\n{resume_text}\n\n"
            "Output the information in a structured format as follows:\n"
            "Skills: [list of skills]\n"
            "Certifications: [list of certifications]\n"
            "Tools: [list of tools]\n"
            "Additional Expertise: [list any extra proficiencies or areas of expertise]\n"
        )
        ...
        response_text = model_response.get("content", [{}])[0].get("text", "").strip()
        if not response_text:
            raise ValueError("Empty or improperly structured response from model.")

        # Parse the response using keywords
        def extract_section(keyword, next_keyword=None):
            if keyword in response_text:
                section = response_text.split(keyword)[1]
                if next_keyword and next_keyword in section:
                    section = section.split(next_keyword)[0].strip()
                return [item.strip("- ").strip() for item in section.split("\n") if item.strip()]
            return []

        skills = extract_section("Skills:", "Certifications:") or ["No Skills Found"]
        certifications = extract_section("Certifications:", "Tools:") or ["No Certifications Found"]
        tools = extract_section("Tools:", "Additional Expertise:") or ["No Tools Found"]
        additional_expertise = extract_section("Additional Expertise:") or ["No Additional Expertise Found"]
        ...
        return {
            ...
            "Tools": ", ".join(tools),
            "Additional Expertise": ", ".join(additional_expertise),
        }
    except Exception as e:
        print(f"ERROR: An error occurred while analyzing {file_path}. Reason: {e}")
        return {
            "File Name": os.path.basename(file_path),
            "Skills": "Error",
            "Certifications": "Error",
            "Tools": "Error",
            "Additional Expertise": "Error",
        }

def analyze_resumes_in_directory(directory_path):
    ...
    required_columns = ["File Name", "Skills", "Certifications", "Tools", "Additional Expertise"]
    for col in required_columns:
        if col not in output_df:
            output_df[col] = "No Data Found"

    # Save to CSV
    output_file = "resumes_analysis_output.csv"
...
def prepare_resume_df_for_ranking(resume_df):
    """Ensure the resume DataFrame has a consistent structure for ranking."""
    ...
    return resume_df[required_columns]
...
def render_client_feedback_page(agent):
    """Render the Client Feedback tab with JD + Feedback drop zones and dynamic labeling"""
    st.markdown(f"<div class='section-header'>💬 Client Feedback Enhancement</div>", unsafe_allow_html=True)

    jd_file = st.file_uploader("📄 Drop or upload a Job Description (TXT or DOCX) [optional]", type=["txt", "docx"], key="jd_upload_client")
    ...
    if generate_button:
        if not feedback_file:
            st.warning("Please upload the client feedback file.")
            return

        # Determine version label based on whether a JD file is also provided
        if jd_file:
            version_label = "Client Enhanced Version"
            try:
            ...
            version_label = "Final Enhanced Version"
            if st.session_state.get("final_version"):
                job_description = st.session_state.final_version
            else:
                st.warning("No job description available (upload a JD file or generate a final version first).")
                return
        ...
        # Construct prompt for the LLM (unchanged except for display label)
        prompt = (
        ...
        # Display the results with dynamic labeling
        st.markdown("---")
        ...
        st.subheader(f"✅ {version_label}")
        st.text_area("Enhanced JD", enhanced_text.strip(), height=400)



def render_jd_optimization_page(logger, analyzer, agent):
    """Render the combined JD Optimization page that merges JD Versions and Feedback Loop functionality."""
    st.markdown("<div class='section-header'>JD Optimization</div>", unsafe_allow_html=True)

    ##########################
    # Part 1: Job Description Selection/Upload
    ##########################
    st.markdown("<div class='subsection-header'>1. Select/Upload Job Description</div>", unsafe_allow_html=True)
    jd_directory = os.path.join(os.getcwd(), "JDs")
    try:
        files = [f for f in os.listdir(jd_directory) if f.endswith(('.txt', '.docx'))]
        file_col, upload_col = st.columns([2, 1])
        with file_col:
            selected_file = st.selectbox("Select Job Description File", files, key="file_selector")
        with upload_col:
            uploaded_file = st.file_uploader("Or Upload New File", type=['txt', 'docx'], key="file_uploader")
        ...
        if uploaded_file:
            ...
            st.error("Please select or upload a Job Description file.")
            return
        
        st.markdown("<div class='subsection-header'>Job Description Preview</div>", unsafe_allow_html=True)
        st.text_area("Original JD", st.session_state.original_jd, height=250, disabled=True)
    except Exception as e:
        st.error(f"Error loading job description: {e}")
        return

    ##########################
    # Part 2: Generate Enhanced Versions & Analysis
    ##########################
    st.markdown("<div class='subsection-header'>2. Generate Enhanced Versions</div>", unsafe_allow_html=True)
    generate_btn = st.button("Generate Enhanced Versions", key="generate_btn")
    if generate_btn or ('enhanced_versions' not in st.session_state):
        with st.spinner("Generating enhanced versions..."):
            ...
            st.session_state.enhanced_versions = versions
            logger.log_versions_generated(versions)
            # If your Streamlit version doesn't have st.experimental_rerun(), remove/replace the call below
            st.rerun()

    if 'enhanced_versions' in st.session_state:
        # Create top-level tabs for Enhanced Versions and Analysis & Comparison
        step2_tabs = st.tabs(["Enhanced Versions", "Analysis & Comparison"])
        
        # 2a: Enhanced Versions
        with step2_tabs[0]:
            st.markdown("<div class='subsection-header'>Enhanced Versions</div>", unsafe_allow_html=True)
            version_tabs = st.tabs(["Version 1", "Version 2", "Version 3"])
            for idx, (tab, version) in enumerate(zip(version_tabs, st.session_state.enhanced_versions)):
                with tab:
                    st.text_area(f"Enhanced Version {idx+1}", version, height=300, disabled=True)
                    st.download_button(
                        label=f"Download Version {idx+1}",
                        data=version,
                        file_name=f"enhanced_jd_version_{idx+1}.txt",
                        mime="text/plain"
                    )
        
        # 2b: Analysis & Comparison
        with step2_tabs[1]:
            st.markdown("<div class='subsection-header'>Analysis & Comparison</div>", unsafe_allow_html=True)
            original_scores = analyzer.analyze_text(st.session_state.original_jd)
            intermediate_scores = {
                f'Version {i+1}': analyzer.analyze_text(v)
                for i, v in enumerate(st.session_state.enhanced_versions)
            }
            all_scores = {'Original': original_scores, **intermediate_scores}
    ...
    ##########################
    # Part 3: Feedback & Final Version Generation
    ##########################
    st.markdown("<div class='subsection-header'>3. Provide Feedback & Generate Final Version</div>", unsafe_allow_html=True)
    
    left_col, right_col = st.columns(2)
    with left_col:
        # Let user select which version they want to refine
        st.markdown("Select a version to base your feedback on:")
        if 'enhanced_versions' in st.session_state:
            selected_version = st.radio("Choose Version", ["Version 1", "Version 2", "Version 3"], key="version_selector")
            selected_index = int(selected_version[-1]) - 1
        else:
            st.info("No enhanced versions found. Please generate enhanced versions first.")
            return
        
        # Removed the text area that shows the selected version content
        ...
        if logger.current_state["feedback_history"]:
            ...
            with st.expander("View Feedback", expanded=False):
                for i, feedback in enumerate(logger.current_state["feedback_history"], 1):
                    feedback_text = feedback.get("feedback", "") if isinstance(feedback, dict) else feedback
                    feedback_type = feedback.get("type", "General Feedback") if isinstance(feedback, dict) else "General Feedback"
                    st.markdown(f"**#{i} - {feedback_type}:** {feedback_text}")

    with right_col:
        st.markdown("Provide your feedback:")
        feedback_types = ["General Feedback", "Rejected Candidate", "Hiring Manager Feedback", "Client Feedback", "Selected Candidate", "Interview Feedback"]
        selected_feedback_type = st.selectbox("Feedback Type", feedback_types, key="feedback_type_selector")
        user_feedback = st.text_area("Enter your suggestions:", height=150, key="user_feedback")

        if st.button("Add Feedback", key="add_feedback"):
            if user_feedback.strip():
                feedback_obj = {
                    "feedback": user_feedback,
                    "type": selected_feedback_type,
                ...
                logger.current_state["feedback_history"].append(feedback_obj)
                logger._save_state()
                st.session_state.feedback_history = st.session_state.get("feedback_history", []) + [feedback_obj]
                st.success("Feedback added!")
                st.rerun()
            else:
                st.warning("Please enter some feedback first.")

    # Button to generate the final enhanced version
    if st.button("Generate Final Enhanced Version", key="generate_final_jd"):
        with st.spinner("Generating Final Enhanced Version..."):
            logger.log_version_selection(selected_index)
            base_description = st.session_state.enhanced_versions[selected_index]
            final_desc = agent.generate_final_description(base_description, logger.current_state["feedback_history"])
            
            st.session_state.current_enhanced_version = final_desc
            st.session_state.final_version_generated = True
            st.session_state.final_version = final_desc
            logger.log_enhanced_version(final_desc, is_final=True)
            st.success("Final version generated!")
            st.rerun()

    # Show the final enhanced version (plus download options) once generated
    if st.session_state.get('final_version_generated'):
        st.markdown("<div class='subsection-header'>Final Enhanced Version</div>", unsafe_allow_html=True)
        final_text = st.session_state.final_version if st.session_state.final_version else ""
        st.text_area("Your Final Enhanced JD", final_text, height=250, disabled=True)
        
        # Download buttons for the final version
        dl_col1, dl_col2 = st.columns(2)
        with dl_col1:
            st.download_button(
                label="Download as TXT",
                data=final_text,
                file_name="final_enhanced_version.txt",
                mime="text/plain"
            )
        with dl_col2:
            if st.button("Download as DOCX"):
                save_enhanced_jd(final_text, "final_enhanced_version.docx", 'docx')
                st.success("Saved as final_enhanced_version.docx")

    # Final Analysis comparing all versions plus the new final version
    if st.session_state.get('final_version_generated'):
        st.markdown("<div class='section-header'>Final Analysis</div>", unsafe_allow_html=True)
        final_versions = {"Original": analyzer.analyze_text(st.session_state.original_jd)}
        for i, v in enumerate(st.session_state.enhanced_versions):
            final_versions[f"Version {i+1}"] = analyzer.analyze_text(v)
        final_versions["Final Enhanced Version"] = analyzer.analyze_text(st.session_state.final_version)
        
        # If a client enhanced version also exists, add it
        if st.session_state.get("client_enhanced_version"):
            final_versions["Client Enhanced Version"] = analyzer.analyze_text(st.session_state.client_enhanced_version)
        
        col_a, col_b = st.columns(2)
        with col_a:
            radar_chart_final = create_multi_radar_chart(final_versions)
            st.plotly_chart(radar_chart_final, use_container_width=True)
        with col_b:
            comparison_df_final = create_comparison_dataframe(final_versions)
            st.dataframe(
                comparison_df_final,
                use_container_width=True,
                height=400,
